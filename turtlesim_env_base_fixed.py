...
class TurtlesimEnvBase(metaclass=abc.ABCMeta):
    ...
    def __init__(self):
        ...
        self.WAIT_AFTER_MOVE = 0.01     # oczekiwanie po setPose() i przed color_api.check() [s] (0.005 też daje radę)
    ...
    def reset(self,
        ...
        for tidx,tname in enumerate(tnames):
            ...
            if sections[tidx]=='default':                   # żółw pozycjonowany wg csv
                ...
                weights = [self.routes[agent.route][i][0] for i in range(len(self.routes[agent.route]))]
                sec_id = random.choices(range(len(self.routes[agent.route])), weights=weights)[0]
    ...
    def get_road(self,tname):
        agent = self.agents[tname]
        #print(tname,agent.color_api.check())
